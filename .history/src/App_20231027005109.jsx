...
function App() {
  ...
  const { setDataFunc, page } = useContext(DataContext);
  ...
  useEffect(debounce(() => {
    function handleScroll() {
      ...
      const check = setDataFunc(page + 1);
      if (!check) window.removeEventListener("scroll", handleScroll);
      if (scrollY + windowHeight >= viewHeight) setDataFunc(page + 1);
    }
    window.addEventListener("scroll", handleScroll);
    return () => {
      window.removeEventListener("scroll", handleScroll);
    };
  },1000), [page, setDataFunc]);
